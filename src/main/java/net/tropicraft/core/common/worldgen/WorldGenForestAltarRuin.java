...
public class WorldGenForestAltarRuin extends TCDirectionalGen {
    ...
    public boolean generate(int i, int j, int k) {
        ...
        for(int z = 0; z < length; z++) {
            ...
            for(int x = 0; x < width; x++) {
                ...
                int y = j;
                this.placeBlockWithDir(x, y, z, logs, 1);

                if(z == 0) {
                    if(x == 0 || x == width - 1) {
                        this.placeBlockWithDir(x, y + 1, z, palmFence, 0);
                        this.placeBlockWithDir(x, y + 2, z, palmFence, 0);
                    } else {
                        this.placeBlockWithDir(x, y + 1, z, slabs, 2);
                        this.placeBlockWithDir(x, y + 2, z, palmFence, 0);
                    }
                } else if(z == 1) {
                    if(x == 0 || x == width - 1) {
                        ...
                        this.placeBlockWithDir(x, y + 1, z, chunk, 0);
                        this.placeBlockWithDir(x, y + 2, z, flowerPot, 0);
                    ...
                    if(x == 0 || x == width - 1) {
                        this.placeBlockWithDir(x, y + 1, z, tikiTorch, 1);
                        this.placeBlockWithDir(x, y + 2, z, tikiTorch, 1);
                        this.placeBlockWithDir(x, y + 3, z, tikiTorch, 0);
                    } else if(x != halfWidth - 1 && x != halfWidth) {
                        this.placeBlockWithDir(x, y + 1, z, palmStairs, this.dir);
                        if(!hasGennedTunnel) {
    ...
    private void generateTunnel(int i, int j, int k) {
        ...
        this.placeBlockWithDir(i, j, k, bambooChest, 0);
    ...
    public ItemStack randLoot()
    {
        ...
        if(picker < 6)
        {
            return new ItemStack(bambooShoot, rand.nextInt(20) + 1);
        }
        else if(picker < 8)
        {
        	return new ItemStack(coconutBomb, rand.nextInt(3) + 1);
        }
        else if(picker < 10)
        {
            return new ItemStack(scale, rand.nextInt(3) + 1);
        }
        else if(picker < 12)
        {
        ...
        {
            return new ItemStack(cookedFrogLeg, rand.nextInt(4) + 1);
        }
		else if(picker == 14)
		{
			return new ItemStack(maskMap, 1, rand.nextInt(7));
		}
		else if(picker == 15)
		{
			return new ItemStack(recordTradeWinds, 1);
		}
		else if(picker == 16)
		{
			return new ItemStack(recordEasternIsles, 1);
		}
        else
        {
            return new ItemStack(dartGun, 1);
        }
    }
